...
function define_inputSizes(model::Model,config::Dict)
	model.inputSizes =get(config,"inputSizes",model.inputSizes)
end

function define_outputSizes(model::Model, config::Dict)
	model.outputSizes = get(config,"outputSizes",model.outputSizes)
end
...
function evaluateRequest(models::Vector)
	function handler(request::HTTP.Request)
		...
		return HTTP.Response(["Content-Type" => "application/json"], JSON.json(body))
	end
	return handler
end
	...
	function handler(request::HTTP.Request)
		...
		try
			if model_inWrt < 1 || model_inWrt > length(model.inputSizes)
				...
				returnHTTP.Response(400, ["Content-Type" => "application/json"], JSON.json(body))
			end
		catch e
			return runtime_error(model, e, "the evaluation of inputSizes", "InputSizes", "input size")
		end             
...
function applyJacobianRequest(models::Vector)
	function handler(request::HTTP.Request)
		...
		returnHTTP.Response(200, ["Content-Type" => "application/json"], JSON.json(body))
	end
	return handler
end
