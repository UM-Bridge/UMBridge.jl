...
function with_logging(handler, log::Bool=false, callName::String="handler")
  ...
  return function(req::HTTP.Request)
    ...
    #println("** Incoming Request (", handler , ") **")  # Indicate handler name
...
function serve_models(models::Vector, port=4242, logging::Bool=false, max_workers=1)
    # @TODO: Different argument for "Evaluate" to prevent logging abuse?
    router = HTTP.Router()
    # router = HTTP.use!(router, print_request)
